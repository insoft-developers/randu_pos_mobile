import '../../../../../data/repositories/i_printer_repository.dart';
import '../../../../entities/printer/printer_folder/printer_folder.dart';
import '../../../../entities/result.dart';
import '../../../usecase.dart';

class SavePrinterFolderUC implements UseCase<Result<String>, PrinterFolder> {
  final IPrinterRepository _repository;

  SavePrinterFolderUC({required IPrinterRepository repository})
      : _repository = repository;

  @override
  Future<Result<String>> call(PrinterFolder request) async {
    var result = await _repository.savePrinterFolder(request);
    return result;
  }
}
